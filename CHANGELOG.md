...
- `baseOpenThreads` in `src/index.ts` now uses a type-predicate filter `(f): f is PreviousFinding & { threadId: string }` instead of a truthy guard followed by a `!` non-null assertion, making the narrowing explicit and compiler-verified ([#769](https://github.com/manki-review/manki/issues/769)).
