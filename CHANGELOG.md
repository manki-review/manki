...
- Extracted `AGENT_POOL`, `TRIVIAL_VERIFIER_AGENT`, and `buildAgentPool` from `src/review.ts` into a new low-level `src/agents.ts` module, breaking the layering inversion where `src/config.ts` (low layer) imported from `src/review.ts` (orchestration layer) just to validate `reviewers` names against the agent pool. `src/config.ts`, `src/index.ts`, and the test files now import these primitives directly from `./agents`, and `src/review.ts` consumes them from the same module so the agent-pool primitives have a single canonical home decoupled from review orchestration ([#676](https://github.com/manki-review/manki/issues/676)).
